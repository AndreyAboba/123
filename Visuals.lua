...
function Visuals.Init(UI, Core, notify)
    ...
    local function initWatermark()
        ...
        local function updateSizes()
            ...
            if WatermarkConfig.showFPS and elements.FPSContainer then
                ...
                Cache.TextBounds.FPSContainer = fpsContainerWidth + 30
            end
            ...
            if WatermarkConfig.showTime and elements.TimeContainer then
                ...
                Cache.TextBounds.TimeContainer = timeContainerWidth + 10
            end
    ...
    Core.Services.RunService.Heartbeat:Connect(function(deltaTime)
        ...
        if Elements.Watermark.FPSLabel and WatermarkConfig.showFPS and Elements.Watermark.FPSFrame.Visible then
        ...
        if Elements.Watermark.TimeLabel and WatermarkConfig.showTime and Elements.Watermark.TimeFrame.Visible then
    ...
    local function updateESP(deltaTime)
        ...
        if ESP.LastFriendsList ~= Core.Services.FriendsList then
            ESP.LastFriendsList = Core.Services.FriendsList
            ESP.FriendsListCache = {}
            for _, player in pairs(Core.Services.Players:GetPlayers()) do
                if player ~= Core.PlayerData.LocalPlayer then
                    ESP.FriendsListCache[player] = Core.Services.FriendsList and Core.Services.FriendsList[player.Name:lower()] or false
                end
            end
        end

        local camera, time = Core.PlayerData.Camera, tick()
        ...
        for _, player in pairs(Core.Services.Players:GetPlayers()) do
            ...
            local isFriend = ESP.FriendsListCache[player]
            if isFriend == nil then
                isFriend = Core.Services.FriendsList and Core.Services.FriendsList[player.Name:lower()] or false
                ESP.FriendsListCache[player] = isFriend
            end

            if rootPart and humanoid and humanoid.Health > 0 then
                local positionChanged = not esp.LastPosition
                local healthChanged = not esp.LastHealth or math.abs(humanoid.Health - esp.LastHealth) > 0.1
                local timeSinceLastUpdate = time - esp.LastUpdateTime
                local shouldUpdate = positionChanged or healthChanged or timeSinceLastUpdate > 0.5

                if not shouldUpdate then continue end

                local rootPos, onScreen = camera:WorldToViewportPoint(rootPart.Position)
                positionChanged = positionChanged or (esp.LastPosition and (rootPos - esp.LastPosition).Magnitude > 0.5)
                shouldUpdate = positionChanged or healthChanged or onScreen ~= esp.LastVisible
                ...
                if onScreen then
                    local headOffset = head and head.Size.Y / 2 + 0.5 or 2
                    local headPos = camera:WorldToViewportPoint(rootPart.Position + Vector3.new(0, headOffset, 0))
    ...
    Core.Services.Players.PlayerRemoving:Connect(function(player)
        hideESP(player)
        ESP.FriendsListCache[player] = nil
    end)

    Core.Services.RunService.Heartbeat:Connect(updateESP)
