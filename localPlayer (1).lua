...
local function SetupUI(UI)
    ...
    -- Отдельная секция для кнопки Sync Config
    if not UI.Tabs.LocalPlayerSettings then
        UI.Sections.LocalPlayerSettings = UI.Tabs.Misc:Section({ Side = "Right", Name = "LocalPlayerSettings" })
    else
        UI.Sections.LocalPlayerSettings = UI.Tabs.LocalPlayerSettings:Section({ Side = "Right", Name = "LocalPlayerSettings" })
    end
    if UI.Sections.LocalPlayerSettings then
        UI.Sections.LocalPlayerSettings:Header({ Name = "Local Player Settings" })
        UI.Sections.LocalPlayerSettings:Button({
            Name = "Sync Config",
            Callback = function()
